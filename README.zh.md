...
在 `revert-v1.3` 上活跃开发。Phase 3 原生运行时已上线——audit / fix / evaluate / merge / analyze / chief 都走 Go runner，opencode 是遗留角色的 fallback（等每个角色都迁完且稳定观察一段时间后会移除）。
...
欢迎 PR。先读 `docs/dev/02_architecture.md`。**Chief 不能动工作队列**这个不变量神圣不可侵犯——如果你觉得 Chief 需要一个新的"改任务"工具，99% 概率答案是 `delegate_to_maintain`。
