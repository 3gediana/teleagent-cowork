...
### 5.2 PR 评审工作流（分支 → main）— 刚修复并验证 ✅

```
Agent 创建并进入分支
  ↓
分支内自由修改（branch.change_submit，无审核）
  ↓
...
人类查看两份评估报告，点击"确认合并"
  ↓
Merge Agent 执行 git merge（--no-commit 试合并）
  → 成功: 版本升级 + 广播 + 分支关闭
  → 失败: 通知人类，PR 回到 evaluated
```
...
| `/api/v1/branch/*` | - | 创建/进入/离开/列表/关闭/sync_main |
| `/api/v1/role/*` | - | 列表/更新模型 |
...
| PR 技术评审 | ✅ | Evaluate Agent → evaluate_output 工具 → 后端处理 ✅ **刚修复** |
| PR 合并 | 🟡 | 代码已实现，待完整测试（Merge Agent + merge_output） |
...
| PR Agent Session 未注册到 DefaultManager | `HandleEvaluateOutput` 无法找到 PR，PR 永远卡在 `evaluating` | 在 `TriggerEvaluateAgent` / `TriggerMergeAgent` 中添加 `RegisterSession()` |
...
| 问题 | 严重程度 | 说明 |
|------|----------|------|
| `branch.change_submit` API 未实现 | 🔴 | MCP 客户端有 `branch` 工具，但后端缺少分支内直接写入文件的 API |
| PR Merge Agent 未完整测试 | 🟡 | 代码已实现，但缺少端到端验证 |
| Maintain Agent PR 业务评估 | 🟡 | `TriggerMaintainBizReview` 代码存在，但流程未完整打通 |
| 前端 PR 审批交互 | 🟡 | PRPage 已存在，但 approve_review/approve_merge 的 UI 反馈可优化 |
| `start.ps1` 路径过时 | 🟡 | 脚本引用的路径是旧目录结构（`backend/` 而非 `platform/backend/`） |
| README 端口过时 | 🟡 | README 写 3303/33303，实际后端 3003，前端端口需确认 |
...
| 功能 | 说明 |
|------|------|
| 分支内自由修改的 MCP 工具完善 | `branch.change_submit` 后端 API 缺失 |
...
1. **实现 `branch.change_submit` 后端 API**
   - Agent 在分支内自由修改，直接写入 branch worktree
   - 无 version 检查、无审核流程
   - 这是 PR 工作流的前提

2. **完整测试 PR 合并流程**
   - 端到端验证：submit → approve_review → evaluate → maintain → approve_merge → merge
   - 测试冲突场景（simple conflict auto-resolve, complex conflict abort）

3. **修复 `start.ps1` 和 README**
   - 更新路径为 `platform/backend/`
   - 确认前端实际端口
...
| `platform/backend/internal/agent/role.go` | 8 个 Agent 角色定义 |
| `platform/backend/internal/service/pr_agent.go` | PR 相关 Agent 触发 + 工具处理 |
